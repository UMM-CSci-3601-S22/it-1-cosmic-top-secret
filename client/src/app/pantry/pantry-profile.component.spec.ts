...
describe('PantryProfileComponent', () => {
  ...
  it('should have `null` for the pantry item with a bad ID', () => {
    activatedRoute.setParamMap({ id: 'badID' });

    // If the given ID doesn't map to a pantry item, we expect the service
    // to return `null`, so we would expect the component's pantry item
    // to also be `null`.
